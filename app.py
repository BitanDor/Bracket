...
def render_sidebar(all_guesses, actual_results, config):
    ...
    for user, user_obj in all_guesses.items():
        ...
        winner = effective_guesses.get("FINAL") or effective_guesses.get("FINALS", "TBD")
        flag = config.TEAM_FLAGS.get(winner, "")
        winner_display = f"{flag} {winner}" if winner != "TBD" else "TBD"
    ...
    if user_name:
            ...
            for m_id in stage_matches:
                ...
                if "TBD" not in participants:
                    ...
                    if actual_winner:
                        ...
                        st.write(f"**{config.ROUND_DICT.get(m_id, m_id)}:** {actual_winner} (נעול)")
                    else:
                    ...
                    st.info(f"{config.ROUND_DICT.get(m_id, m_id)}: מחכה לתוצאות...")
        ...
        if st.button("שמור ניחוש"):
            ...
            for m, v in new_guesses.items():
                if "PLAY_IN" in m:  # תיקון חינם לפליי-אין
                    user_obj[m] = v
                    for b in [f"corrections_after_{s}" for s in config.STAGES if "PLAY_IN" in s]:
                        if b in user_obj: user_obj[b].pop(m, None)
                elif active_bucket == "base":
                    user_obj[m] = v
